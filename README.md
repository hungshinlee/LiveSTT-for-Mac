...
長時間、有固定主題、術語密集的場合（授課、研討會、技術簡報）建議用這三個選項。
以「國語授課、外籍學生看英文字幕」為例：

```bash
uv run livestt -e qwen -l zh --translate-to en -u overlay \
  --terms ai-course.txt \
  --context "這是一堂深度學習課程，會談到 Transformer、注意力機制與訓練技巧" \
  --log "class-$(date +%F).srt"
```

### `--terms`：一個檔案，兩邊生效

辨識與翻譯需要的詞彙高度重疊，分成兩個檔案維護很累贅。`--terms` 用一份檔案
同時餵給兩端：
...
各位同學 = everyone        ← 有 = ：左邊做辨識偏置，整組做翻譯對照
梯度下降 = gradient descent
加權平均 = weighted average
```

有譯法的詞**兩邊都會生效** —— 聽錯了，譯法再準也沒用。

### `--context`：一句話說明主題

同時提供給辨識與翻譯，減少歧義。沒有它的時候，「第三季」在片段中會被翻成
`season three`（電視劇），有了財報脈絡才會是 `third quarter`。

> `apple` 引擎沒有可放自由文字的欄位（`SFSpeechRecognizer` 只吃詞彙清單），
> 所以 `--context` 對它的辨識端無效，但翻譯端仍然生效。

### `--translate-window`：翻譯的跨句記憶

帶上前 N 句作為脈絡，讓代名詞與術語前後一致，預設 `2`。設 `0` 則每句獨立翻譯。

前一句辨識錯誤時可能連帶影響後續，需要切斷時設 `0`。

### 實測效果

AI 課堂情境八句（中英夾雜，`say` 合成）：

| | 基準 | 加上三個選項 |
|---|---|---|
| 各位同學 | "**Gentlemen**" ❌ | "**Everyone**" ✅ |
| 跟 BERT base 一樣 | "the same as **Bird Base**" ❌ | "the same as **Bert base**" ✅ |
| 公式符號 | "q…k…d_k" | "**Q**…**K**…**D_K**" ✅ |
| 延遲 | 0.40 秒／句 | **0.43 秒／句** |

修掉實質錯誤，延遲只增加 7%。

### 長時間穩定性

模擬一小時授課量（200 句連續辨識加翻譯）實測：

| | 結果 |
|---|---|
| 記憶體 | **完全持平**，peak RSS 固定 6.89 GB |
| 延遲漂移 | +5.9%（0.416 → 0.441 秒），屬雜訊範圍 |

搭配 `--log` 把逐字稿存成 SRT，課後可直接配合錄影，或讓學生複習。
...
| `--terms` | | 課程詞彙表，辨識與翻譯共用 | 無 |
| `--context` | | 一句話描述主題，提供給辨識與翻譯 | 無 |
...
| `--translate-window` | | 翻譯帶上前 N 句脈絡，0 為獨立翻譯 | `2` |
