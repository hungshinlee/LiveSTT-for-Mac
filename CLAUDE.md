...
`postprocess.EXCEPTIONS` 在 OpenCC 之後還原專有名詞的「台」。
**「臺」一律照教育部標準**（臺北、舞臺、電視臺），只有這些例外：
語言名稱（臺灣台語，教育部 2024 年定名刻意混用兩字）、人名（郭台銘）、
公司登記名稱（台積電、台塑、台玻…）—— 它們本來就寫「台」，改掉等於寫錯名字。

兩字的例外**必須帶否定回顧**。少了它，「電視臺電話」會被改成「電視台電話」
（「臺電」其實是「電視臺」的尾字接「電話」的頭字），「電視臺語音」同理。
`_TAIL_CHARS` 列出「臺」作為詞尾時前面常見的字；新增兩字例外時要一起考慮。

這張表**只收有官方或登記依據的詞**，不要拿它做一般性的用詞替換 ——
那正是我們不用 s2twp 的理由。
...
`test_docs.py` 比對文件與程式是否同步（選項、預設值、模組清單、連結、範例指令）。
改了 CLI 或新增模組卻沒更新文件時，它會直接讓測試失敗。
單獨執行是 `uv run python scripts/check_docs.py`。
**新增測試檔或模組時記得同步更新 README 的專案結構**，否則這項會擋下來。
